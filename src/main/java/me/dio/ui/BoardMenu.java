...
public class BoardMenu {
    ...
    private void moveCardToNextColumn() throws SQLException {
        ...
        var boardColumnsInfo = entity.getBoardColumns().stream()
                ...
                .toList();
        try (var connection = getConnection()) {
            new CardService(connection).moveToColumn(cardId, boardColumnsInfo);
        ...
        try (var connection = getConnection()) {
            new CardQueryService(connection).findById(selectedCardId)
                    .ifPresentOrElse(c -> {
                                ...
                                System.out.println(c.blocked() ? "Esta bloqueado. Motivo: " + c.blockReason() : "Nao está bloqueado.");
                                System.out.printf("Ja foi bloqueado %s vezes\n", c.blockAmount());
                                System.out.printf("Esta no momento na coluna %s -%s\n", c.columnId(), c.columnName());
                            },
                            () -> System.out.printf("Nao existe card com o id %s\n", selectedCardId));
