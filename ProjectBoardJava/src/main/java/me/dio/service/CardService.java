...
public class CardService {
    ...
    public void moveToColumn(final Long cardId, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException {
        try {
            var dao = new CardDAO(connection);
    ...
    public void block(final Long id, final String reason, final List<BoardColumnInfoDTO> boardColumnsInfo)throws SQLException{
        try{
            ...
            if (dto.blocked()) {
                var message = "O card %s ja está bloqueado".formatted(id);
                throw new CardBlockedException(message);
            }
            var currentColumn = boardColumnsInfo.stream()
    ...
    public void unblock(final Long id, final String reason)throws SQLException{
        try{
            ...
            var dto = optional.orElseThrow(
                    () -> new EntityNotFoundException("O card de id %s nao foi encontrado".formatted(id)));
            if (!dto.blocked()) {
                var message = "O card %s nao está bloqueado".formatted(id);
                throw new CardBlockedException(message);
            }
            var blockDAO = new BlockDAO(connection);
